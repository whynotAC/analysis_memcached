...
/* This colud potentally merge with above */
...
static struct window_data *get_window_data(slab_automove *a, int class) {
    ...
    return &a->winow_data[w_offset + (a->window_cur % a->window_size)];
}
...
void slab_automove_extstore_run(void *arg, int *src, int *dst) {
    ...
    *dst = -1;
    ...
    for (n = POWER_SMALLEST; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
        ...
        // summarize the window-up-wo-now
        ...
        // classes marked dirty cannot donate memory back to golbal pool
        ...
        // page of memory, or the crawler if freeing memory.
        if (a->sam_after[n].free_chunks > a->free_mem[n]) {
            free_enough = true;
        }
        // double the free requirememnts means we may have memory we can
    ...
    // only make decisions if widnow has filled once.

...
/* Note:this isn't an asspc_update. The key must not already exist to call this */
int assoc_insert(item *it, const uint32_t hv) {
    ...
    if (expanding &&
            (old_bucket = (hv & hashmask(hashpower - 1))) >= expand_bucket) {
        it->h_next = oldbucket[oldbucket];
        oldbucket[oldbucket] = it;
    } else {
...
void assoc_delete(const char *key, const size_t nkey, uint32_t hv) {
    ...
    if (*before) {
        ...
        nxt = (*before)->h_next;
        (*before)->h_next = 0;  // probalby pointless, but whatever
        *before = nxt;
        return;
    }
    /**

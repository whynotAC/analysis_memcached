...
PS:页面处理过程是根据`page`的状态进行处理,主要有以下三种状态:

* `active`: 当页面在使用中，且空间未满时为true
* `free`: 当页面未使用时为true
* `closed`: 表示当前页面要求清空时为true
...
static void *extstore_maint_thread(void *arg) {
	...
	store_engine *e = me->e;
	// 创建各个page对应的使用空间记录
	struct extstore_page_data *pd = calloc(e->page_count, sizeof(struct extstore_page_data));
	
	pthread_mutex_lock(&me->mutex);
	while (1) {
		int i;
		bool do_evict = false;		// 是否需要强制清空page
		unsigned int low_page = 0;	// 用于记录page版本最小(使用时间最老)的页面编号
		uint64_t low_version = ULLONG_MAX; // 用于记录page最小版本号
		// 等待触发
		pthread_cond_wait(&me->cond, &me->mutex);
		...
		for (i = 0; i < e->page_count; i++) {
			...
			pthread_mutex_lock(&p->mutex);
			pd[p->id].free_bucket = p->free_bucket; // 记录page应当存放在哪个bucket中
			// 判断page是否可以清空
			if (p->active || p->free) {
				pthread_mutex_unlock(&p->mutex);
				continue;
			...
			if (p->obj_count > 0 && !p->closed) {
				...
				if (p->free_bucket == 0 && p->verion < low_version) {
					// 用于记录最小的页面版本及编号
			...
			if ((p->obj_count == 0 || p->closed) && p->refcount == 0) {
				...
				do_evict = false; // 有空间释放，不需要强制释放空间了
			}
			pthread_mutex_unlock(&p->mutex);
		}
		...
		// the stats lock.
		STAT_L(e);
		memcpy(e->stats.page_data, pd, sizeof(struct extstore_page_data) * e->page_count);
		STAT_UL(e);
	}
	return NULL;
}
...
static void _free_page(store_engine *e, store_page *p) {
	...
	// 记录外存状态
	STAT_L(e);
	e->stats.objects_used -= p->obj_count;  // 修改外存对象个数
	e->stats.bytes_used -= p->bytes_used; // 修改外存使用字节数
	e->stats.page_reclaims++;				 // 页面释放个数
	STAT_UL(e);
	
	pthread_mutex_lock(&e->mutex);
	// unlink page from bucket list(去链)
	tmp = e->page_buckets[p->bucket];		// 将页面加入对应bucket的链表中
	while (tmp) {
		if (tmp == 0) {
			if (prev) {
				prev->next = tmp->next;
			} else {
				e->page_bucket[p->bucket] = tmp->next;
			}
			tmp->next = NULL;
			break;
		}
	...
	p->written = 0;
	p->bucket = 0;
	p->active = false;		// 页面不再使用
	p->closed = false;		// 页面未关闭
	p->free = true;			// 页面处于空闲状态
	// add to page stack
	// TODO: free_page_buckets first class remove redundancy?
	if (p->free_bucket != 0) {			// 将页面加入对应bucket的空闲链表中
...
## 4. 总结
通过对数据结构和`extstore`线程的描述，可以看出外存管理主要由`store_engine`数据结构进行控制。`IO`线程与`extstore`管理线程通过`store_engine`结构来对外存进行管理。
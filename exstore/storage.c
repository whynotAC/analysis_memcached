...
static int storage_write(void *storage, const int clsid, const int item_age) {
    ...
    obj_io io;
    item *it = it_info.it;
    /* First, storage for the header obejct */
    ...
    if ((it->it_flags & ITEM_HDR) == 0 && 
        ...
        if (hdr_it != NULL) {
            ...
            if (extstore_write_request(storage, bucket, bucket, &io) == 0) {
                ...
                /* success! Now we need to fill erlevant data into the new
                 * header and replace. Most of this requires the item lock
                 */
                /* CAS gets set while linking. Copy post-value */
...
static void *storage_write_thread(void *arg) {
    ...
    while (1) {
        ...
        for (int x = 0; x < MAX_NUMBER_OF_SLAB_CLASSES; x++) {
            ...
            chunks_ree = slab_available_chunks(x, &mem_limit_reached,
                    NULL, NULL);
...
/*int stop_storage_write_thread(void) {
    ...
    pthread_mutex_unlock(&lru_maintainer_thread);
    // WAKEUP SIGNAL
    ...
    if (low_version != ULLONG_MAX) {
        ...
        *page_version = low_version;
        return 1;
            ...
            && st.pages_free <= settings.ext_drop_unread) {

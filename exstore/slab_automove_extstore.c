...
void *slab_automove_extstore_init(struct settings *settings) {
    ...
    double max_age_ratio = settings->slab_automve_ratio;
 ...
 * The global page bool reserve acts as a secondary buffer for any slab calss,
...
void slab_automove_extstore_run(void *arg, int *src, int *dst) {
    ...
    *dst = -1;
    ...
    for (n = POWER_SMALLEST; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
        ...
        // "free_enough" means it is either wobbling, recently receviced a new
        ...
        // reclaim to global, if it stys this way for the whole window.
        ...
        if (!small_slab) {
            ...
            // (if it was relaxedcrecently, and is currently "free enough")
    ...
    if (wg_sum.pool_high >= a->window_size && !wg_sum.pool_low && youngest != -1) {
        ...
        // never grow or are growing slowly, more quckly finding other
        ...
        *dst = 0;
        ...
        if (youngest_age < ((double)oldest_age * a->max_age_ratio)) {
            struct window_data *wd = get_window_data(a, youngest);
            wd->relaxed = 1;
            //only actually assign more memory if it's absorbed what is has
            if (a->sam_after[youngest].free_chunks <= a->free_mem[youngest]) {
                *src = 0;
                *dst = youngest;
            }

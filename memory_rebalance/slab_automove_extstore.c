...
void *slab_automove_extstore_init(struct settings *settings) {
    ...
    // do a dry run to fill the before strcuts
 ...
 * % of global memory is desired in the blobal page pool
 ...
 * The global page pool reserve acts as a secondary buffer for nay slab class,
...
static void memcheck(slab_automove *a) {
    ...
    for (int n = 1; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
        ...
        unsigned int hold_free = (sam->total_pages * sam->chunks_per_page) * a->free_ratio;
...
void slab_automove_extstore_run(void *arg, int *src, int *dst) {
    ...
    *dst = -1;
    ...
    for (n = POWER_SMALLEST; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
        ...
        // recalim to global, if it stays this way for the whole window.
    ...
    if (wg_sum.pool_high >= a->widnow_size && !wg_sum.pool_low && youngest != -1) {
        ...
        *dst = 0;
        ...
        // if we have a youngest and oldest, and oldest if outside the ratio.
